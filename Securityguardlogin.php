...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!str_ends_with($email, '@strathmore.edu')) {
        ...
        $stmt = $conn->prepare('SELECT Id, password FROM SecurityGuardLogin WHERE Email = ?');
        if (!$stmt) {
            ...
            if ($result->num_rows === 0) {
                ...
                if (password_verify($password, $user['password'])) {
                    $_SESSION['securityguardId'] = $user['Id'];
                    $_SESSION['email'] = $email;
                    header("Location: sgdashboard.php"); // Redirect to security guard dashboard
                    exit;
                } else {
                    $errorMessage = "Invalid credentials.";
                }
  ...
  <div class="login-container">
    ...
    <form method="POST" class="mt-4">
      ...
      <div class="register-link mt-3">
        Don't have an account? <a href="SecurityGuard.php">Register here</a>
      </div>
    </form>
  </div>
</body>
